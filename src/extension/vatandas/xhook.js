...
xhook.after(function(request, response) {
  let requestURLS=[
  "vatandas.uyap.gov.tr/main/jsp/vatandas/dosya_evrak_bilgileri_brd.ajx",
  ];
  

  let found = requestURLS.find(element => element.indexOf(request.url)>=0);
  if (found!=null) {
    let lastPart=found.match(/(?!.*\/).+(?=\.)/);
    if(lastPart!=null){
    UYAP_EXT.DB[lastPart[0]](request,response);
    }
  }
